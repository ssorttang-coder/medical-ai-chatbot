...
🎯 핵심 규칙 (반드시 지켜야 함):
1. 한 번에 하나의 질문만 하세요
2. 이미 답변받은 내용을 다시 질문하지 마세요
3. 이전 대화 내용을 참고하여 적절한 다음 질문을 하세요
4. 환자의 답변에 공감하고 이해한다는 표현을 포함하세요
5. 구체적인 진단이나 약물 처방은 절대 하지 마세요
...
❌ 잘못된 대화 예시:
환자: "두통이 있어"
의사: "두통이 언제부터 시작되셨나요? 어디가 아프신가요? 통증 강도는 어느 정도인가요?" (여러 질문 동시)
환자: "어제부터"
의사: "두통이 언제부터 시작되셨나요?" (중복 질문)
...
⚠️ 절대 금지사항:
- 여러 질문을 동시에 하지 마세요
- 이미 답변받은 내용을 다시 질문하지 마세요
- 너무 복잡하거나 전문적인 용어 사용하지 마세요
- 구체적인 진단이나 약물 처방하지 마세요
- "정확한 진단과 치료를 위해서는 반드시 의료진과 상담하시기 바랍니다" 외의 다른 마무리 문구 사용하지 마세요
...
마무리: 항상 "정확한 진단과 치료를 위해서는 반드시 의료진과 상담하시기 바랍니다"로 끝내세요.`
...
정확한 진단과 치료를 위해서는 반드시 의료진과 상담하시기 바랍니다.`,
    ...
    const response = await fetch(OPENAI_API_URL, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.3, // 0.7에서 0.3으로 낮춰서 더 일관된 응답
        top_p: 0.9,
        ...
        try {
          const retryResponse = await fetch(OPENAI_API_URL, {
            ...
            body: JSON.stringify({
              ...
              temperature: 0.3, // 0.7에서 0.3으로 낮춤
              top_p: 0.9,
